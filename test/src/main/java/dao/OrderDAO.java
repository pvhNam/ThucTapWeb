...
public class OrderDAO {
	...
	// Lấy danh sách đơn hàng của 1 User (Cho trang Lịch sử mua hàng)
	...
	public List<Order> getAllOrders() {
		...
		String sql = "SELECT o.*, u.fullname, u.phonenumber FROM orders o " + "JOIN users u ON o.user_id = u.uid "
	...
	public List<OrderDetail> getDetails(int orderId) {
		...
		String sql = "SELECT d.*, p.* FROM order_details d " + "JOIN product p ON d.product_id = p.pid "
				+ "WHERE d.order_id = ?";
		try {
			conn = new DBConnect().getConnection();
			ps = conn.prepareStatement(sql);
