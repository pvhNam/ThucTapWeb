...
public class CartController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) 
        ...
        // Kiểm tra lại điều kiện mã (đề phòng đơn hàng thay đổi)
        if (appliedVoucher != null) {
            if (cart.isEmpty()) {
                session.removeAttribute("appliedVoucher");
                session.removeAttribute("voucherMsg"); // Xóa thông báo cũ
            } 
            else if (subtotal < appliedVoucher.getMinOrder()) {
                session.removeAttribute("appliedVoucher");
                // Báo lỗi vào Session để hiển thị ra JSP
                session.setAttribute("voucherMsg", "Mã " + appliedVoucher.getCode() + " đã bị hủy do đơn hàng chưa đủ " + (long)appliedVoucher.getMinOrder() + "đ");
                session.setAttribute("msgType", "error"); // Loại thông báo: lỗi
            } 
            else {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) 
        ...
        // 3. CÁC LOGIC KHÁC (ADD/REMOVE/UPDATE GIỎ HÀNG) - Giữ nguyên
        int pid = 0;
        try {
            if(request.getParameter("pid") != null) pid = Integer.parseInt(request.getParameter("pid"));
        } catch (Exception e) {}
        ...
        if (currentUser != null) {
            ...
            if (cart == null) cart = new ArrayList<>();
            // (Giữ nguyên logic session cart...)
            if ("add".equals(action)) { /* Logic thêm cart session */ }
