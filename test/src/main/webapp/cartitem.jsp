<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<%@ page
	import="java.util.List, model.cartItem, model.user, model.Voucher"%>
			...
			<div class="cart-summary-section">
				<div class="summary-box">
					...
					<div class="voucher-container">
						<label class="voucher-label"><i class="fa-solid fa-ticket"></i>
							Mã ưu đãi của bạn</label>
						...
						<div class="voucher-list">
							...
							<p class="empty-voucher-text">
								Vui lòng <a href="login.jsp">đăng nhập</a> để xem mã ưu đãi.
							</p>
							<%
							} else if (myVouchers == null || myVouchers.isEmpty()) {
							%>
							<p class="empty-voucher-text">
								Bạn chưa lưu mã giảm giá nào.<br> <a href="index.jsp"
									style="font-size: 12px; text-decoration: underline;">Săn mã
									ngay</a>
							</p>
							<%
							} else {
							...
							<div class="voucher-item <%=isApplied ? "active" : ""%>">
								<div class="v-left">
									<span class="v-code"><%=v.getCode()%></span> <span
										class="v-desc"><%=v.getDescription()%></span>
								</div>
								<div class="v-right">
									<%
									if (isApplied) {
						...
						<details class="manual-input">
							...
							<form action="cart" method="post" class="voucher-form-manual">
								<input type="hidden" name="action" value="apply_voucher">
								<input type="text" name="voucherCode" placeholder="Nhập mã..."
									required>
								<button type="submit">
					...
					<div class="summary-row">
						<span>Tạm tính</span> <span><%=df.format(subtotal)%></span>
					</div>
					<%
					if (discount > 0) {
					%>
					<div class="summary-row discount">
						<span>Đã giảm (<%=appliedVoucher.getCode()%>)
						</span> <span>- <%=df.format(discount)%></span>
					</div>
					<%
					}
					%>
					...
					<div class="summary-row total">
						<span>Tổng cộng</span> <span><%=df.format(finalTotal)%></span>
					</div>
